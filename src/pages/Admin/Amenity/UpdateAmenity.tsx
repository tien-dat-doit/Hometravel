...
export default function UpdateAmenity({ modal19, setModal19, amenityTitle1 }: any) {
    ...
    useEffect(() => {
        const fetchAmenity = async () => {
            try {
                const response: any = await amenityAPI.getAllAmenityID(
                    {
                        sortOrder: 'DESC',
                        amenitieTitleId : amenityTitle1.id
                    });
    ...
    const handleSubmit = async (values: FormikValues) => {
        try {
            ...
            for (const record of recordsData) {
                const response: any = await amenityAPI.editAmenitys(record.id, record);
            }
            setModal19(false);
            setIsUpdate(true);
        ...
        try {
            if (window.confirm('Xác nhận xóa toàn bộ tiện ích?')) {
                for (const policy of recordsData) {
                    await amenityAPI.deleteAmenitys(policy.id);
                }
                const response = await amenityAPI.deleteAmenityTitle(amenityTitle1.id);
    ...
    const handleDeletePolicys = async (idPolicys: string) => {
        try {
            ...
            const response = await amenityAPI.deleteAmenitys(idPolicys);
            toast.success('Xóa Tiện Ích Thành Công');
            setModal19(false);
            setIsUpdate(true);
            }
