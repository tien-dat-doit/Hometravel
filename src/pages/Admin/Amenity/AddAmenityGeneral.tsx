...
export default function ModalUpdate({ modal18, setModal18 }: any) {
    ...
    const addAmenity = async (amenityTitleData: any, amenitieData: any) => {
        try {
            const amenityTitleResponse = await amenityAPI.createAmenityGeneralAmenitysTitle(amenityTitleData);
    
            const amenityTitleId = amenityTitleResponse.data.id;
    
            const amenitieResponse = await Promise.all(amenitieData.map(async (amenityData :any) => {
                amenityData.generalAmenitieTitleId = amenityTitleId;
    
                return await amenityAPI.createAmenitysGeneralAmenitys(amenityData);
            }));
            setIsUpdate(true);
            toast.success('Tạo Tiện Ích Thành Công');
            return { amenityTitleResponse, amenitieResponse };
    ...
    const handleSubmit = (values: FormikValues) => {
        if (formData.amenityTitle.name !== ''){
            addAmenity(formData.amenityTitle, formData.amenitieData);
        }
                                            ...
                                            {({ values,handleChange }) => (
                                                <Form>
                                                    ...
                                                    <div className="mt-4">
                                                        <div>
                                                            ...
                                                            <input
                                                                ...
                                                                value={formData.amenityTitle.name}
                                                    ...
                                                    {formData.amenitieData.map((amenity, index) => (
                                                        <div key={amenity.id} className="mt-2">
