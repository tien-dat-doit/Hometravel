...
export default function AddMoreAmenity({ modal17, setModal17, amenityTitle3 }: any) {
    ...
    const addAmenity = async (amenitieData: any) => {
        try {
            const promises = amenitieData.map((amenity : any)=> amenityAPI.createAmenitys(amenity));
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, index: number) => {
        ...
        setFormData(prevState => {
            ...
            newamenitieData[index] = {
                ...newamenitieData[index],
                [name]: value,
                amenitieTitleId: amenityTitle3.id,
