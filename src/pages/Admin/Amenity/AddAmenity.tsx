...
export default function ModalUpdate({ modal17, setModal17 }: any) {
    ...
    const addBlock = () => {
        ...
        setFormData(prevState => ({
            ...
            amenitieData: [
                ...
                { id: '', name: '', status: 'ACTIVE', amenitieTitleId: '' },
            ],
        }));
    };
    ...
    const addAmenity = async (amenityTitleData: any, amenitieData: any) => {
        try {
            const amenityTitleResponse = await amenityAPI.createAmenityTitle(amenityTitleData);
            ...
            const amenitieResponse = await Promise.all(amenitieData.map(async (amenityData :any) => {
                amenityData.amenitieTitleId = amenityTitleId;
    
                return await amenityAPI.createAmenitys(amenityData);
            }));
            setIsUpdate(true);
